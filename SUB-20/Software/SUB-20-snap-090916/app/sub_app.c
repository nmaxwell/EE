...
*-------------------------------------------------------------------
* Version: $Id: sub_app.c,v 1.49 2009/08/27 17:31:33 avr32 Exp $
*-------------------------------------------------------------------
*/
#include <stdio.h>
#include <stdlib.h>
...
int main( int argc, char* argv[] )
{
    ...
    */
    for( cmd=0; cmd<config.todo_sz; cmd++ )
    {
        ...
        case 'D':
            /* GPIO Write */
            rc = sub_gpio_config( fd, config.gpio_val, &tmp, config.gpio_mask );
            if( !rc )
            {
        ...
        case 'O':
            /* GPIO Write */
            rc = sub_gpio_write( fd, config.gpio_val, &tmp, config.gpio_mask );
            if( !rc )
            {
        ...
        case 'W':
            rc = sub_i2c_write( fd, config.sa, config.ma, config.ma_sz, 
                                                    config.buf, config.sz );
            break;
